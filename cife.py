    ...
    for i in tqdm(range(chunk)):
        ...
        for j in range(N):
            if j==i:
                ...
                cmi.append(CMI(F[:, i + start],F[:, j], C))
        mis.append(mi)
        cmis.append(cmi)
...
if __name__=='__main__':
    ...
    for i in tqdm(range(1, K)):
        ...
        J = mic[M] - (mis[M][:, selected_indices] - cmis[M][:, selected_indices]).sum(axis=-1)
        selected_indices.append(M[np.argmax(J)])
        if (i + 1) % 100 == 0:
            print(selected_indices)
            sqlite_util.createSelectedTable(table_name='cife')
            try:
                sqlite_util.read_from_selectecd(False, task_name, i + 1, label, table_name='cife')
                sqlite_util.updata_selected(task_name, i + 1, str(selected_indices), label, table_name='cife')
            except Exception:
                sqlite_util.insert_selected(task_name, i + 1, str(selected_indices), label, table_name='cife')