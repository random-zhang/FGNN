...
for i in tqdm(range(1,K )):
    ...
    print(mic.shape)
    _1=np.expand_dims(mic[M],axis=1)
    _1=np.repeat(_1,len(selected_indices),axis=1)
    _2=cmis[M][:,selected_indices]
    J=mic[M]-(_1-_2).mean(axis=-1)
    selected_indices.append(M[np.argmax(J)])
    if (i+1)%100==0 :
        print(selected_indices)
        sqlite_util.createSelectedTable(table_name='jmi')
        try:
            sqlite_util.read_from_selectecd(False,task_name, i+1,label,table_name='jmi')
            sqlite_util.updata_selected(task_name, i+1,str(selected_indices),label,table_name='jmi')
        except Exception:
            sqlite_util.insert_selected(task_name, i+1,str(selected_indices),label,table_name='jmi')
