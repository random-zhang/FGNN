...
def calculate_cmi_wrapper(process,chunk ,start, N,F,result_dict,C):
...
def s(label=None):
    ...
    for i in range(num_cores):
        ...
        if i == num_cores - 1:
            chunk = N - i * chunk
        process = multiprocessing.Process(target=calculate_cmi_wrapper, args=(i, chunk, start, N, F, result_dict,C))
        process.start()
    ...
    for i in tqdm(range(1, K)):
        ...
        if (i + 1) % 100 == 0:
            ...
            sqlite_util.createSelectedTable()
            try:
                sqlite_util.read_from_selectecd(False, task_name, i + 1, label)
                sqlite_util.updata_selected(task_name, i + 1, str(selected_indices), label)
            except Exception:
                sqlite_util.insert_selected(task_name, i + 1, str(selected_indices), label)
