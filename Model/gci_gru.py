...
class GATLayerWithGRUAndWeight(nn.Module):

    def __init__(self, args, in_features, out_features, concat=True):
        super(GATLayerWithGRUAndWeight, self).__init__()
        self.args = args
        ...
        self.fc_c = nn.GRU(args.max_atom, args.gat_ci_out , batch_first=True, bidirectional=True)

    def forward(self, input, adj, weight, smiles, no=None):
...
class GATLayerWithGRU(nn.Module):

    def __init__(self, args, in_features, out_features, concat=True):
        super(GATLayerWithGRU, self).__init__()
        self.args = args
        self.in_features = in_features
        self.out_features = out_features
        self.concat = concat
